...
exports.insertPenjualan = arg => {
  ...
  return new Promise((resolve, reject) => {
    ...
    db.all(queryStock, idBarang, (err, result) => {
      ...
      stock =
        arg.param == "pembelian"
          ? parseInt(stock) + parseInt(result[0].barang_stock)
          : parseInt(result[0].barang_stock - parseInt(stock));
      if (err) {
        console.log(err);
      } else {
        let query =
          "INSERT INTO tb_penjualan (penjualan_nama, penjualan_nik," +
        ...
        db.run(query, arg.data, (err, result) => {
          if (err) {
            ...
            getLastId().then(id => {
              ...
              // })
              if (idBarang != 5) {
                let queryUpdateStock =
                  "UPDATE tb_barang SET barang_stock = ? WHERE barang_id = ?";
...
exports.getPenjualan = arg => {
  ...
  if (arg.byStatus != null) {
    query += ` AND tb_penjualan.penjualan_status_pembayaran = "${
      arg.byStatus
    }"`;
  }
  ...
  query +=
    " ORDER BY date(tb_penjualan.penjualan_insert_tanggal) DESC, penjualan_id DESC";
...
exports.totalPenjualan = () => {
  let query =
    "select tb_kategori.kategori_id, tb_kategori.kategori_nama, sum(penjualan_jumlah_barang) as total_penjualan  from tb_penjualan join tb_barang, tb_kategori where tb_barang.barang_id = tb_penjualan.penjualan_nama_barang and tb_kategori.kategori_id = tb_barang.barang_category group by tb_barang.barang_category";
  return new Promise((resolve, reject) => {
    db.all(query, (err, result) => {
      if (err) {
        console.log(err);
...
exports.updatePenjualan = arg => {
  let query =
    "UPDATE tb_penjualan SET penjualan_total_bayar = ?, penjualan_status_pembayaran = ?, penjualan_insert_tanggal = ? WHERE penjualan_id = ?";
  return new Promise((resolve, reject) => {
...
exports.getTotalPiutang = () => {
  let query =
    'SELECT COUNT(*) as bon FROM tb_penjualan WHERE penjualan_status_pembayaran = "Belum"';
  return new Promise((resolve, reject) => {
    db.all(query, (err, result) => {
      if (err) {
        console.log(err);
...
function insertKonsumen(param) {
  let query = `SELECT * FROM tb_konsumen WHERE konsumen_nik = "${
    param.nik
  }" AND barang_id = ${param.barangId}`;
  return new Promise((resolve, reject) => {
    db.all(query, (err, result) => {
      if (err) {
        console.log(err);
...
function getLastId() {
  let query =
    "SELECT penjualan_id FROM tb_penjualan ORDER BY penjualan_id DESC limit 1";
  return new Promise((resolve, reject) => {
    db.all(query, (err, result) => {
      if (err) {
        console.log(err);
